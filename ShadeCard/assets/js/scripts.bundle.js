...
$(document).ready(function () {
    ...
    //Menu fixing for closing problem with dropdown and submenu
    $(".kt-header__topbar-wrapper").click(function () {
        //alert($(".kt-menu__item").hasClass("kt-menu__item--open-dropdown kt-menu__item--hover"));
        if ($(".kt-menu__item").hasClass("kt-menu__item--open-dropdown kt-menu__item--hover")) {
            $(".kt-menu__item").removeClass("kt-menu__item--open-dropdown kt-menu__item--hover");
        }
    });


    //$('tr.custom-pagination > td').attr('colspan', '12');
    //$('tr.custom-pagination > td:not(:last-child)').remove();

    $('.children-entry > tbody > tr:last-child > td').attr('colspan', '8');
    $('.children-entry > tbody > tr:last-child > td:not(:last-child)').remove();
    ...
    $("#myInput1").on("keyup", function () {
        ...
        $(".myTable1 tr:not(:first-child)").filter(function () {
            $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
        });
    });
    $("#myInput2").on("keyup", function () {
        var value = $(this).val().toLowerCase();
        $(".myTable2 tr:not(:first-child)").filter(function () {
            $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
        });
    });
    $("#myInput3").on("keyup", function () {
        var value = $(this).val().toLowerCase();
        $(".myTable3 tr:not(:first-child)").filter(function () {
            $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
        });
    });

    $("#myDivSrch").on("keyup", function () {
        var value = $(this).val().toLowerCase();
        $(".myInputdiv").filter(function () {
            $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
        });
    });
    ...
    $(".profile-short-info.insert-pipeline span:empty").remove();
    $('.hide-on-empty-val span').each(function () {
        if ($(this).is(":empty")) {
            $(this).closest('i').remove();
        }
    });
    $(".hide-on-empty-val span:empty").parent().hide();
    $(".insert-comma > span").append('<span>,</span>');
    $(".insert-pipeline span").append('<span> | </span>');



    //var myDiv1Para = $('.move-to-a').remove();
    //myDiv1Para.appendTo('.move-to-a-target');





    //$('.hide-on-empty-val').not($('span').parent()).hide();
});
