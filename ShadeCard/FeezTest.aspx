<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FeezTest.aspx.cs" Inherits="ShadeCard.FeezTest" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <meta charset="utf-8" />
    <title>GridViewScroll</title>
<%--    <link href="css/web.css" rel="stylesheet" />
    <script type="text/javascript" src="js/gridviewscroll.js"></script>--%>
<link href="assets/css/web.css" rel="stylesheet" />
<script src="assets/js/gridviewscroll.js"></script>
    <script type="text/javascript">
        var gridViewScroll = null;
        window.onload = function () {
            gridViewScroll = new GridViewScroll({
                elementID: "gvMain",
                width: 850,
                height: 350,
                freezeColumn: true,
                freezeFooter: true,
                freezeColumnCssClass: "GridViewScrollItemFreeze",
                freezeFooterCssClass: "GridViewScrollFooterFreeze",
                freezeHeaderRowCount: 2,
                freezeColumnCount: 3,
                onscroll: function (scrollTop, scrollLeft) {
                    console.log(scrollTop + " - " + scrollLeft);
                }
            });
            gridViewScroll.enhance();
        }
        function getScrollPosition() {
            var position = gridViewScroll.scrollPosition;
            alert("scrollTop: " + position.scrollTop + ", scrollLeft: " + position.scrollLeft);
        }
        function setScrollPosition() {
            var scrollPosition = { scrollTop: 50, scrollLeft: 50};

            gridViewScroll.scrollPosition = scrollPosition;
        }
    </script>
</head>
<body>
    <form id="form1" runat="server">
      
    <table cellspacing="0" id="gvMain" style="width: 100%; border-collapse: collapse;">
        <tr class="GridViewScrollHeader">
            <td colspan="3" style="background-color: #F4F4F4;">Product</td>
            <td colspan="2" style="background-color: #F4F4F4;">Package</td>
            <th rowspan="2" scope="col">StandardCost</th>
            <th rowspan="2" scope="col">ListPrice</th>
            <th rowspan="2" scope="col">SafetyStockLevel</th>
            <th rowspan="2" scope="col">SellStartDate</th>
            <th rowspan="2" scope="col">SellEndDate</th>
            <th rowspan="2" scope="col">ModifiedDate</th>
        </tr>
        <tr class="GridViewScrollHeader">
            <th scope="col">ProductID</th>
            <th scope="col">Name</th>
            <th scope="col">Number</th>
            <th scope="col">ReorderPoint</th>
            <th scope="col">Weight</th>
        </tr>
        <tr class="GridViewScrollItem">
            <td>747</td>
            <td>HL Mountain Frame - Black, 38</td>
            <td>FR-M94B-38</td>
            <td>375</td>
            <td>2.68</td>
            <td>739.0410</td>
            <td>1349.6000</td>
            <td>500</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>&nbsp;</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>748</td>
            <td>HL Mountain Frame - Silver, 38</td>
            <td>FR-M94S-38</td>
            <td>375</td>
            <td>2.68</td>
            <td>747.2002</td>
            <td>1364.5000</td>
            <td>500</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>&nbsp;</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>749</td>
            <td>Road-150 Red, 62</td>
            <td>BK-R93R-62</td>
            <td>75</td>
            <td>15.00</td>
            <td>2171.2942</td>
            <td>3578.2700</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2002 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>750</td>
            <td>Road-150 Red, 44</td>
            <td>BK-R93R-44</td>
            <td>75</td>
            <td>13.77</td>
            <td>2171.2942</td>
            <td>3578.2700</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2002 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>751</td>
            <td>Road-150 Red, 48</td>
            <td>BK-R93R-48</td>
            <td>75</td>
            <td>14.13</td>
            <td>2171.2942</td>
            <td>3578.2700</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2002 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>752</td>
            <td>Road-150 Red, 52</td>
            <td>BK-R93R-52</td>
            <td>75</td>
            <td>14.42</td>
            <td>2171.2942</td>
            <td>3578.2700</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2002 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>753</td>
            <td>Road-150 Red, 56</td>
            <td>BK-R93R-56</td>
            <td>75</td>
            <td>14.68</td>
            <td>2171.2942</td>
            <td>3578.2700</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2002 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>754</td>
            <td>Road-450 Red, 58</td>
            <td>BK-R68R-58</td>
            <td>75</td>
            <td>17.79</td>
            <td>884.7083</td>
            <td>1457.9900</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2002 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>755</td>
            <td>Road-450 Red, 60</td>
            <td>BK-R68R-60</td>
            <td>75</td>
            <td>17.90</td>
            <td>884.7083</td>
            <td>1457.9900</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2002 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>756</td>
            <td>Road-450 Red, 44</td>
            <td>BK-R68R-44</td>
            <td>75</td>
            <td>16.77</td>
            <td>884.7083</td>
            <td>1457.9900</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2002 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>757</td>
            <td>Road-450 Red, 48</td>
            <td>BK-R68R-48</td>
            <td>75</td>
            <td>17.13</td>
            <td>884.7083</td>
            <td>1457.9900</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2002 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>758</td>
            <td>Road-450 Red, 52</td>
            <td>BK-R68R-52</td>
            <td>75</td>
            <td>17.42</td>
            <td>884.7083</td>
            <td>1457.9900</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2002 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>759</td>
            <td>Road-650 Red, 58</td>
            <td>BK-R50R-58</td>
            <td>75</td>
            <td>19.79</td>
            <td>486.7066</td>
            <td>782.9900</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2003 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
        <tr class="GridViewScrollItem">
            <td>760</td>
            <td>Road-650 Red, 60</td>
            <td>BK-R50R-60</td>
            <td>75</td>
            <td>19.90</td>
            <td>486.7066</td>
            <td>782.9900</td>
            <td>100</td>
            <td>7/1/2001 12:00:00 AM</td>
            <td>6/30/2003 12:00:00 AM</td>
            <td>3/11/2004 10:01:36 AM</td>
        </tr>
    </table>
    </form>
</body>
</html>
